            ...
            <tr>
                ...
                <td>年龄 
                  <select name="age" size="1" id="select2">
                    <?php for($i = 18; $i <= 50; $i ++ ):?>
                    <option <?=$i==$rec->age?'selected' : ($i==21?'selected':'')?> value="<?=$i?>"><?=$i?></option>
                    <?php endfor?>
                </select>&nbsp;&nbsp;
                身高
                  <select name="height" size="1" id="select2">
                    <?php for($i = 155; $i <= 190; $i ++ ):?>
                    <option value="<?=$i?>" <?=$i==$rec->height?'selected' : ($i==178?'selected':'')?> value="<?=$i?>"><?=$i?>cm</option>
                    <?php endfor?>      
                  </select>&nbsp;&nbsp;
                体重
                  <select name="weight" size="1" id="select2">
                    <?php for($i = 40; $i <= 100; $i ++ ):?>
                    <option value="<?=$i?>" <?=$i==$rec->weight?'selected' : ($i==63?'selected':'')?> value="<?=$i?>"><?=$i?>kg</option>
                    <?php endfor?>
                  </select>
                </td>
            </tr>
