...
class Curl {
    ...
    public function setCookie($name, $value, $domain, $path = '/') {
        if (isset($this->_cookies[$domain])) {
            array_push($this->_cookies[$domain], array('name' => $name, 'value' => $value, 'path' => $path));
        }
        else {
            $this->_cookies[$domain] = array(array('name' => $name, 'value' => $value, 'path' => $path));
        }
    }
    ...
    private function _attach_cookie($url) {
        ...
        if (isset($this->_cookies[$sub_domain])) {
            foreach($this->_cookies[$sub_domain] as $cookie) {
                if (strpos(dirname($this->_url['path']), $cookie['path']) === 0) {
